...
export class EmailAuthenticationComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.seo.updatePageMetadata({ title: await this.translate.get('VIEWS.TWO_FACTOR_AUTHENTICATION.AUTHENTICATOR.EMAIL_TAB_TITLE').toPromise() });
  ...
  async submit6DigitCode(): Promise<void> {
    ...
    try {
      ...
      const response = await this.http.put<any>(`${environment.params.host}/api/authentication/account/security/2fa/email/authentication-code/check`, { code: this.code, session_data: this.session_data }).toPromise();
